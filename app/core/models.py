...
class Project(Base):
    ...
    modules = relationship("ProjectModule", back_populates="project", lazy="selectin")
    files = relationship("ProjectFile", back_populates="project", lazy="selectin")
    functions = relationship("ProjectFunction", back_populates="project", lazy="selectin")
    ide_sessions = relationship("IDESession", back_populates="project", lazy="selectin")
    change_logs = relationship("ChangeLog", back_populates="project", lazy="selectin")
...
class ProjectModule(Base):
    """Project module/layer: backend, frontend, mobile, bot, etc."""
    ...
    name = Column(String(255), nullable=False)  # backend, frontend, mobile, bot, shared
    module_type = Column(
        String(50), nullable=False
    )  # backend | frontend | mobile | bot | shared | infra | docs
    path = Column(String(1000), nullable=False)  # relative path within project
    description = Column(Text, nullable=True)
    tech_stack = Column(JSONB, nullable=True, default=list)  # ["fastapi", "postgresql"]
    frameworks = Column(JSONB, nullable=True, default=list)  # ["react", "nextjs"]
    languages = Column(JSONB, nullable=True, default=list)  # ["python", "typescript"]
    entry_points = Column(JSONB, nullable=True, default=list)  # main files
    config_files = Column(JSONB, nullable=True, default=list)  # package.json, requirements.txt
    total_files = Column(Integer, default=0, nullable=False)
    total_lines = Column(Integer, default=0, nullable=False)
    metadata_ = Column("metadata", JSONB, nullable=True, default=dict)
    is_active = Column(Boolean, default=True, nullable=False)
    ...
    files = relationship("ProjectFile", back_populates="module", lazy="selectin")

    __table_args__ = (
        Index("ix_project_modules_project", "project_id"),
        Index("ix_project_modules_type", "module_type"),
    )


class ProjectFile(Base):
    """Detailed file information with full content analysis."""
    ...
    module_id = Column(
        UUID(as_uuid=True), ForeignKey("project_modules.id", ondelete="SET NULL"), nullable=True
    )
    component_id = Column(
    ...
    size_bytes = Column(Integer, default=0, nullable=False)
    line_count = Column(Integer, default=0, nullable=False)
    imports = Column(JSONB, nullable=True, default=list)  # imported modules/packages
    exports = Column(JSONB, nullable=True, default=list)  # exported symbols
    dependencies = Column(JSONB, nullable=True, default=list)  # file dependencies
    summary = Column(Text, nullable=True)  # AI-generated summary
    purpose = Column(Text, nullable=True)  # what this file does
    embedding = Column(Vector(get_settings().embedding_dim), nullable=True)
    metadata_ = Column("metadata", JSONB, nullable=True, default=dict)
    is_active = Column(Boolean, default=True, nullable=False)
    ...
    module = relationship("ProjectModule", back_populates="files")
    component = relationship("Component")
    functions = relationship("ProjectFunction", back_populates="file", lazy="selectin")

    __table_args__ = (
        Index("ix_project_files_project", "project_id"),
        Index("ix_project_files_module", "module_id"),
        Index("ix_project_files_path", "path"),
        Index("ix_project_files_hash", "content_hash"),
        Index(
            "ix_project_files_embedding",
            "embedding",
            postgresql_using="hnsw",
            postgresql_ops={"embedding": "vector_cosine_ops"},
            postgresql_with={"m": 16, "ef_construction": 64},
...
class ProjectFunction(Base):
    """Function/method/class detailed analysis."""
    __tablename__ = "project_functions"

    id = Column(UUID(as_uuid=True), primary_key=True, default=_new_uuid)
    file_id = Column(
        UUID(as_uuid=True), ForeignKey("project_files.id", ondelete="CASCADE"), nullable=False
    )
    project_id = Column(
        UUID(as_uuid=True), ForeignKey("projects.id", ondelete="CASCADE"), nullable=False
    )
    ...
    signature = Column(Text, nullable=True)  # full signature
    docstring = Column(Text, nullable=True)  # documentation
    body = Column(Text, nullable=True)  # function body
    line_start = Column(Integer, nullable=False)
    line_end = Column(Integer, nullable=False)
    parameters = Column(JSONB, nullable=True, default=list)  # [{name, type, default}]
    return_type = Column(String(255), nullable=True)
    decorators = Column(JSONB, nullable=True, default=list)  # @decorator names
    calls = Column(JSONB, nullable=True, default=list)  # functions this calls
    called_by = Column(JSONB, nullable=True, default=list)  # functions that call this
    complexity = Column(Integer, default=1, nullable=False)  # cyclomatic complexity
    ...
    # Relationships
    file = relationship("ProjectFile", back_populates="functions")
    project = relationship("Project", back_populates="functions")

    __table_args__ = (
        Index("ix_project_functions_file", "file_id"),
        Index("ix_project_functions_project", "project_id"),
        Index("ix_project_functions_name", "name"),
        Index("ix_project_functions_type", "type"),
        Index(
            "ix_project_functions_embedding",
            "embedding",
            postgresql_using="hnsw",
            postgresql_ops={"embedding": "vector_cosine_ops"},
            postgresql_with={"m": 16, "ef_construction": 64},
...
class IDESession(Base):
    """Track IDE sessions: which IDE, model, user actions."""
    ...
    ide_name = Column(String(100), nullable=False)  # cursor | windsurf | vscode | antigravity
    ide_version = Column(String(50), nullable=True)
    model_name = Column(String(100), nullable=True)  # gpt-4 | claude-3 | llama3.2
    model_provider = Column(String(100), nullable=True)  # openai | anthropic | ollama
    user_id = Column(String(255), nullable=True)  # optional user identifier
    machine_id = Column(String(255), nullable=True)  # machine identifier
    ...
    # Relationships
    project = relationship("Project", back_populates="ide_sessions")
    changes = relationship("ChangeLog", back_populates="ide_session", lazy="selectin")

    __table_args__ = (
        Index("ix_ide_sessions_project", "project_id"),
        Index("ix_ide_sessions_ide", "ide_name"),
        Index("ix_ide_sessions_model", "model_name"),
        Index("ix_ide_sessions_active", "is_active"),
    )


class ChangeLog(Base):
    """Track all changes: what changed, who changed, when, why."""
    ...
    ide_session_id = Column(
        UUID(as_uuid=True), ForeignKey("ide_sessions.id", ondelete="SET NULL"), nullable=True
    )
    file_id = Column(
    ...
    diff = Column(Text, nullable=True)  # unified diff
    lines_added = Column(Integer, default=0, nullable=False)
    lines_removed = Column(Integer, default=0, nullable=False)
    commit_hash = Column(String(64), nullable=True)  # git commit if available
    commit_message = Column(Text, nullable=True)
    branch_name = Column(String(255), nullable=True)
    ide_name = Column(String(100), nullable=True)
    model_name = Column(String(100), nullable=True)
    model_provider = Column(String(100), nullable=True)
    prompt_used = Column(Text, nullable=True)  # what prompt triggered this change
    ...
    ide_session = relationship("IDESession", back_populates="changes")
    file = relationship("ProjectFile")

    __table_args__ = (
        Index("ix_change_logs_project", "project_id"),
        Index("ix_change_logs_session", "ide_session_id"),
        Index("ix_change_logs_file", "file_id"),
        Index("ix_change_logs_type", "change_type"),
        Index("ix_change_logs_created", "created_at"),
        Index("ix_change_logs_commit", "commit_hash"),
    )
